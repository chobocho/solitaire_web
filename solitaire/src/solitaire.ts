...
export type GameState = 'idle' | 'play' | 'pause' | 'win' | 'fail';
...
export class SolitaireGame {
  ...
  hasAnyMove(): boolean {
    ...
    for (let fi = DeckIndex.Found1; fi <= DeckIndex.Found4; fi++) {
      ...
      for (let t = DeckIndex.Tab1; t <= DeckIndex.Tab7; t++) {
        if (this._tableauAcceptsRule(this._decks[t] as TableauDeck, f)) { accepted = true; break; }
      }
      if (!accepted) continue;

      const fitsOnF = (c: Card): boolean =>
        c.color !== f.color && c.number === f.number - 1;

      if (pileCards.some(fitsOnF)) return true;
      ...
      for (let from = DeckIndex.Tab1; from <= DeckIndex.Tab7; from++) {
        ...
        const seqLen = src.sequenceLength();
        for (let k = 0; k < seqLen; k++) {
          if (fitsOnF(src.get(size - 1 - k))) return true;
        }
      }
    }
  ...
  undo(): boolean {
    ...
    if (this._state !== 'play' && this._state !== 'pause') return false;
  ...
  fail(): void {
    if (this._state === 'play' || this._state === 'pause') this._state = 'fail';
  }
